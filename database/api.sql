...
SELECT * FROM data.ingredient;

CREATE VIEW api.container_type AS
SELECT * FROM data.container_type;

CREATE VIEW api.label_preset AS
SELECT * FROM data.label_preset;

CREATE VIEW api.portion AS
SELECT id, batch_id, created_at, expiry_date, status, consumed_at
FROM data.portion;

CREATE VIEW api.event AS
SELECT * FROM data.event;

-- Batch summary: batches grouped with frozen/consumed counts
...
FROM data.batch b
JOIN data.portion p ON p.batch_id = b.id
LEFT JOIN data.image i ON i.id = b.image_id
...
SELECT
    ...
    encode(i.image_data, 'base64') AS image
FROM data.portion p
JOIN data.batch b ON b.id = p.batch_id
LEFT JOIN data.image i ON i.id = b.image_id;

-- Freezer history: daily running totals
...
date_range AS (
    SELECT generate_series(
        COALESCE((SELECT MIN(created_at) FROM data.portion), CURRENT_DATE),
        CURRENT_DATE,
        '1 day'::interval
    )::date AS date
)
...
SELECT
    ...
    COALESCE(
        (SELECT string_agg(ri.ingredient_name::TEXT, ', ' ORDER BY ri.ingredient_name)
         FROM data.recipe_ingredient ri
    ...
    encode(i.image_data, 'base64') AS image
FROM data.recipe r
LEFT JOIN data.image i ON i.id = r.image_id;
